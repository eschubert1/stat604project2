...
hist(arraymeta$age, main="Distribution of observed ages", xlab="Age")
...
```{r}
# 62 controls
sum(substr(colnames(genemeta),1,4)=="AFFX")
...
heatmap(expression[ix,], Rowv = NA, Colv = NA, revC = TRUE, scale = "column", labRow = region.symb, labCol= NA)
...
pheatmap(mat = exp_hmap, color = colors, scale="column", cluster_rows=FALSE, cluster_cols=FALSE, show_rownames = TRUE, show_colnames=F)

heatmap(expression[ix,], Rowv = NA, Colv = NA, revC = TRUE, scale = "column", labRow = labregion.symb, labCol=NA)
```
